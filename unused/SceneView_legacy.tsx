...
export const SceneView = forwardRef<SceneViewHandle, {}>(({}, ref) => {
  ...
  useEffect(() => {
    ...
    albedoTextureRef.current = new THREE.CanvasTexture(albedoCanvasRef.current);
  ...
  const updateTextures = async () => {
    ...
    // textureUtilsのupdateAllCanvasesを叩く
    updateAllCanvases(
      {
        bump: bumpCanvasRef.current.getContext("2d")!,
        roughness: roughnessCanvasRef.current.getContext("2d")!,
        albedo: albedoCanvasRef.current.getContext("2d")!,
      },
      {
        wood: woodImageRef.current || undefined,
        frame: frameImagesRef.current[useTagStore.getState().Inputs.frameType],
      },
    );
    ...
    if (albedoTextureRef.current) albedoTextureRef.current.needsUpdate = true;

    setIsLoading(false);
  };

  // updateTexture実行のdebounce化
  ...
  useEffect(() => {
    ...
    // inputs の変更を監視して、変更があったらテクスチャを更新する
    // subscribe(selector, callback) を使う
    const unsubscribe = useTagStore.subscribe(
      (state) => state.Inputs, // Inputsが変わった時だけ反応
      () => {
        // ストアが変わったら、debounceされた関数を叩く
        debouncedUpdate();
      },
